...
void backbone(
    float *x, LFM2Config *config, Weights *weights, Buf *buf,
    int batch, int seq_len, int d_model, int k_size, int layer_index
){
    ...
    if (is_attn_check[layer_index]) {
        gqattention(x, config, &weights->gqa[layer_index], buf, batch, seq_len);
    } else {
        gscblock(x, &weights->gsc[layer_index], buf, batch, seq_len, d_model, k_size);
    }
    x = buf->x_out;
